...
from pulser.register.weight_maps import WeightMap, DetuningMap
...
def test_init(layout:RegisterLayout, register:Register, map_reg:MappableRegister):
    str_key = {"1":1.0}
    wrong_key = {4:1.0}
    ...
    for bad_key in (str_key, wrong_key):
        ...
        for reg in (layout, map_reg):
            ...
            with pytest.raises(
                ValueError,
                match=("The trap ids of detuning weights have to be integers"
                        " between 0 and 4"),
            ):
                reg.define_detuning_map(bad_key)
        with pytest.raises(
            ValueError,
            match=("The qubit ids of detuning weights have to be defined in the"
                " register."),
        ):
            register.define_detuning_map(bad_key)
    with pytest.raises(
        ValueError,
        match="Number of traps and weights don't match."):
    ...
    for reg in (layout, map_reg, register):
        ...
        with pytest.raises(ValueError, match="The sum of the weights should be 1."):
            reg.define_detuning_map(bad_sum)
        for map in (det_map, slm_map):
            detuning_map = reg.define_detuning_map(map)
            assert np.all(map[i] == detuning_map.weights[i] for i in range(len(map)))
            assert np.all(layout.coords[i] == detuning_map.trap_coordinates[i] for i in range(len(map)))
...
def test_draw(det_map, slm_map, patch_plt_show):
    ...
    with pytest.raises(ValueError, match="masked qubits and dmm qubits must be the same."):
        slm_map._draw_2D(
            slm_map._initialize_fig_axes(np.array(slm_map.trap_coordinates))[1],
...
def test_DMM():
    ...
    for value in (dmm.max_abs_detuning, dmm.min_retarget_interval, dmm.fixed_retarget_t, dmm.max_targets):
        assert value is None
    with pytest.raises(ValueError, match="bottom_detuning must be negative."):
        DMM(bottom_detuning=1)
    assert dmm._has_fixed_addressing == True
    with pytest.raises(NotImplementedError, match=f"{DMM} cannot be initialized from `Global` method."):
        DMM.Global(None, None, bottom_detuning=1)
    with pytest.raises(NotImplementedError, match=f"{DMM} cannot be initialized from `Local` method."):
