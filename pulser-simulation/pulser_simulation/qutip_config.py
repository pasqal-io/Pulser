...
class QutipConfig(EmulationConfig[QutipState]):
    ...
    def __init__(
        ...
        *,
        ...
        default_evaluation_times: Sequence[float] | Literal["Full"] = "Full",
        **backend_options: Any,
    ):
        """Initializes a QutipConfig."""
        if backend_options.setdefault("interaction_matrix"):
    ...
    def _validate_noise_model(self, noise_model: NoiseModel) -> None:
        """Checks whether the noise model is supported."""
        noise_types = noise_model.noise_types
        if (
            ("doppler" in noise_types)
            or ("SPAM" in noise_types and noise_model.state_prep_error)
            or ("amplitude" in noise_types and noise_model.amp_sigma)
        ):
            raise NotImplementedError(
                "'QutipBackendV2' does not support stochastic noise."
            )
