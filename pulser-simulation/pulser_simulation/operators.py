...
class QuditOperator:
    ...
    def from_coeffs_operators(
        ...
        """Defines a QuditOperator from coefficients and operators."""
    ...
    def build_operator(self):
        """Builds the operator associated with the QuditOperator."""
        return sum(
            [
                self.operation[0]
                * build_operator(
                    self.sampled_seq.eigenbasis,
                    [0],
                    [(operation[1], [0])],
                    self.operators,
                )
...
class MultiQuditOperator:
    ...
    def __init__(
        ...
        """Initializes a MultiQuditOperator."""
        ...
        if not isinstance(operations[0][0], Number):
            ...
            raise ValueError(
                "Operations should be a Sequence of tuple[QuditOperator, str] "
                "or a Sequence of tuple[Number, "
                "Sequence[tuple[QuditOperator, str]]]."
            )
        self.operations_operators = [
            operation[1] for operation in self.operations
        ...
        for operations_operator in self.operations_operators:
            ...
            if len(eigenbases) > 1:
                raise ValueError(
                    "All the eigenbases defined in QuditOperators should "
                    "match."
                )
            ...
            for qudit_operator in qudit_operators:
                for (
                    ...
                    if operator_label not in self.operators:
                        ...
                        raise ValueError(
                            f"Operator {operator_label} is defined differently"
                            " in two QuditOperators."
                        )
    ...
    def from_coeffs_operators(
        ...
        """Defines a MultiQuditOperator from a MultiQuditOperator."""
        return QuditOperator(
            qubit_ids, zip(operations_coeffs, operations_operators)
        )

    def build_operator(self) -> qutip.Qobj:
        """Builds the Qutip.Qobj associated with the MultiQuditOperator."""
        return sum(
            [
                self.operation[0]
                * build_operator(
