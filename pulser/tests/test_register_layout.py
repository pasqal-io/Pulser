...
def test_mappable_register_creation():
    ...
    assert mapp_reg.find_indices({"q2", "q4", "q1"}, ["q4", "q2","q1","q2"]) == [2,1,0,1]

    with pytest.raises(ValueError, match="must be selected among pre-declared qubit IDs"):
        mapp_reg.find_indices({"q2", "q4", "q1", "q5"}, ["q4", "q2","q1","q2"])

    with pytest.raises(ValueError, match="must be selected among the chosen IDs"):
        mapp_reg.find_indices({"q2", "q4", "q1"}, ["q4", "q2","q1","q2", "q3"])
